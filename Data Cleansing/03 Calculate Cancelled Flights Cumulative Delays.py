...
cumulative_arr_cancelled = crs_arr.groupby(['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'])['ArrDelayMinutes'].sum().unstack(2).stack().groupby(level=[0,1]).cumsum()  # ARR_DELAY_NEW is arrival delays that are positive

# sort based on date and hour:
cumulative_dep_cancelled = cumulative_dep_cancelled.sort_index(level=(0,1))
cumulative_arr_cancelled = cumulative_arr_cancelled.sort_index(level=(0,1))
cumulative_dep_cancelled = cumulative_dep_cancelled.reset_index()
cumulative_arr_cancelled = cumulative_arr_cancelled.reset_index()

# Merge datasets together
cumulative_final_df = pd.merge(cumulative_arr_cancelled, cumulative_dep_cancelled,  how='left', left_on=['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'], right_on = ['Marketing_Airline_Network','CRS_DEP_DATE','CRS_DEP_HOUR'])
cumulative_final_df = cumulative_final_df[['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR','0_x','0_y']]

cumulative_final_df.columns = ['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR','CumulativeArrDelayCANCELLED','CumulativeDepDelayCANCELLED']
cumulative_final_df = cumulative_final_df.fillna(method='ffill')

final_df_2 = pd.merge(final_df, cumulative_final_df,  how='left', left_on=['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'], right_on = ['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'])
final_df_2.columns


# Fill in NAs using the same methodology
final_df_2['CRS_ARR_DATE'] = pd.to_datetime(final_df_2['CRS_ARR_DATE'])
final_df_2['Weekday'] = final_df_2['CRS_ARR_DATE'].dt.dayofweek # Add weekday variable
final_df_2['DepDelayCANCELLED'] = round(final_df_2['DepDelayCANCELLED'],0)
final_df_2['ArrDelayCANCELLED'] = round(final_df_2['ArrDelayCANCELLED'],0)
final_df_2.to_csv('daily_&_cumulative_final_data_cancelled.csv')

final_df_WN = final_df_2[final_df_2['Marketing_Airline_Network']=='WN']
final_df_WN.to_csv('daily_&_cumulative_final_data_cancelled_WN.csv')
