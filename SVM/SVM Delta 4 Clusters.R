...
dataDL = read.csv("testing_cumulative_data_DL.csv")

#attach(dataAS)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataDL, SplitRatio = .7)
train <- subset(dataDL, sample == TRUE)
test <- subset(dataDL, sample == FALSE)

write.csv(train, "train_DL_4_clusters.csv")

trainUpdated = read.csv("updated_train_DL.csv")
...
write.csv(pred, "DL_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "Y Test Data DL.csv")


########################### START HERE 

### consider confusion matrices for initial visual
classificationsDL = read.csv("predicted-actual-DL.csv")

predicted=classificationsDL$Predicted
actual=classificationsDL$Actual
