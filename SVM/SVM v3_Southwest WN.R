...
dataWN = read.csv("testing_cumulative_data_WN_original clusters.csv")

attach(dataWN)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataWN, SplitRatio = .7)
train <- subset(dataWN, sample == TRUE)
test <- subset(dataWN, sample == FALSE)

write.csv(train, "train_WN_original clusters.csv")

trainUpdated = read.csv("updated_train_WN_original clusters.csv")
...
write.csv(pred, "WN_SVM Predictions_First8hrPreds_original clusters.csv")
write.csv(y_test, "Y Test Data_WN.csv")

### consider confusion matrices for initial visual
classificationsWN = read.csv("predicted-actual-WN.csv")

predicted=classificationsWN$Predicted
actual=classificationsWN$Actual
