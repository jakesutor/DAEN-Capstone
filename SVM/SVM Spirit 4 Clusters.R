...
dataNK = read.csv("testing_cumulative_data_NK.csv")

#attach(dataAS)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataNK, SplitRatio = .7)
train <- subset(dataNK, sample == TRUE)
test <- subset(dataNK, sample == FALSE)

write.csv(train, "train_NK_4_clusters.csv")

trainUpdated = read.csv("updated_train_NK.csv")
...
write.csv(pred, "NK_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "Y Test Data NK.csv")


########################### START HERE 

### consider confusion matrices for initial visual
classificationsNK = read.csv("predicted-actual-NK.csv")

predicted=classificationsNK$Predicted
actual=classificationsNK$Actual
