...
dataUA = read.csv("testing_cumulative_data_UA.csv")

#attach(dataAS)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataUA, SplitRatio = .7)
train <- subset(dataUA, sample == TRUE)
test <- subset(dataUA, sample == FALSE)

write.csv(train, "train_UA_5_clusters.csv")

trainUpdated = read.csv("updated_train_UA.csv")
...
write.csv(pred, "UA_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "Y Test Data UA.csv")


########################### START HERE 

### consider confusion matrices for initial visual
classificationsUA = read.csv("predicted-actual-UA.csv")

predicted=classificationsUA$Predicted
actual=classificationsUA$Actual
