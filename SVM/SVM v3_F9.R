...
dataF9 = read.csv("testing_cumulative_data_F9_original clusters.csv")

attach(dataF9)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataF9, SplitRatio = .7)
train <- subset(dataF9, sample == TRUE)
test <- subset(dataF9, sample == FALSE)

write.csv(train, "train_F9_original clusters.csv")

trainUpdated = read.csv("updated_train_F9.csv")
...
write.csv(pred, "F9_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "Y Test Data_F9.csv")

### consider confusion matrices for initial visual
classificationsF9 = read.csv("predicted-actual-F9.csv")

predicted=classificationsF9$Predicted
actual=classificationsF9$Actual
