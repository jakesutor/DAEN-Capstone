...
dataG4 = read.csv("testing_cumulative_data_G4_original clusters.csv")

attach(dataG4)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataG4, SplitRatio = .7)
train <- subset(dataG4, sample == TRUE)
test <- subset(dataG4, sample == FALSE)

write.csv(train, "train_G4_original_clusters.csv")

trainUpdated = read.csv("updated_train_G4.csv")
...
write.csv(pred, "G4_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "Y Test Data.csv")

### consider confusion matrices for initial visual
classificationsG4 = read.csv("predicted-actual-G4.csv")

predicted=classificationsG4$Predicted
actual=classificationsG4$Actual
