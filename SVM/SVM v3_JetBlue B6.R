...
dataB6 = read.csv("testing_cumulative_data _ w predictors _ B6v2.csv")

attach(dataB6)
set.seed(321)

#Split the data into a training & test set (30/70)
sample <- sample.split(dataB6, SplitRatio = .7)
train <- subset(dataB6, sample == TRUE)
test <- subset(dataB6, sample == FALSE)

write.csv(train, "train_B6_original_clusters.csv")

trainUpdated = read.csv("updated_train_B6.csv")
...
write.csv(pred, "B6_SVM Predictions_First8hrPreds.csv")
write.csv(y_test, "B6_Y Test Data.csv")

### consider confusion matrices for initial visual
classificationsB6 = read.csv("predicted-actual-B6.csv")

predicted=classificationsB6$Predicted
actual=classificationsB6$Actual
