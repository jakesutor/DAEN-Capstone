# -*- coding: utf-8 -*-
"""
Created on Sun Jun 28 15:43:17 2020
...
data = data[['CRS_DEP_DATETIME_EST','CRS_DEP_DATE','CRS_DEP_HOUR','CRS_ARR_DATETIME_EST','CRS_ARR_DATE','CRS_ARR_HOUR','MKT_UNIQUE_CARRIER','ORIGIN','ORIGIN_CITY_NAME','DEST','DEST_CITY_NAME','CANCELLED']]
data.columns = ['CRS_DEP_DATETIME_EST','CRS_DEP_DATE','CRS_DEP_HOUR','CRS_ARR_DATETIME_EST','CRS_ARR_DATE','CRS_ARR_HOUR','Marketing_Airline_Network','Origin','OriginCityName','Dest','DestCityName','CANCELLED']

# Create csv from dataframe
data.to_csv('cleaned_cancelled_data.csv')

# Divide into departure and arrival datasets
crs_dep = data[['CRS_DEP_DATETIME_EST','CRS_DEP_DATE','CRS_DEP_HOUR','Marketing_Airline_Network','Origin','OriginCityName','Dest','DestCityName','CANCELLED']]
crs_arr = data[['CRS_ARR_DATETIME_EST','CRS_ARR_DATE','CRS_ARR_HOUR','Marketing_Airline_Network','Origin','OriginCityName','Dest','DestCityName','CANCELLED']]
...
dep_cancelled = crs_dep.groupby(['Marketing_Airline_Network','CRS_DEP_DATE','CRS_DEP_HOUR'])['CANCELLED'].sum().unstack(2).stack()  # ARR_DELAY_NEW is arrival delays that are positive
arr_cancelled = crs_arr.groupby(['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'])['CANCELLED'].sum().unstack(2).stack()  # ARR_DELAY_NEW is arrival delays that are positive

# cumulative count of cancelled flights
dep_cancelled = crs_dep.groupby(['Marketing_Airline_Network','CRS_DEP_DATE','CRS_DEP_HOUR'])['CANCELLED'].sum().unstack(2).stack().groupby(level=[0,1]).cumsum()  # ARR_DELAY_NEW is arrival delays that are positive
arr_cancelled = crs_arr.groupby(['Marketing_Airline_Network','CRS_ARR_DATE','CRS_ARR_HOUR'])['CANCELLED'].sum().unstack(2).stack().groupby(level=[0,1]).cumsum()  # ARR_DELAY_NEW is arrival delays that are positive
